...
void queue_destroy(queue_t *q) {
    // Завершение мониторингового потока
    if (q != NULL) {
        int err = pthread_cancel(q->qmonitor_tid); // Отправляем сигнал завершения
        if (err != 0) {
            fprintf(stderr, "queue_destroy: pthread_cancel() failed: %s\n", strerror(err));
        }

        // Ожидаем завершение потока, чтобы избежать утечек ресурсов
        err = pthread_join(q->qmonitor_tid, NULL);
        if (err != 0) {
            fprintf(stderr, "queue_destroy: pthread_join() failed: %s\n", strerror(err));
        }

        // Очистка всех узлов в очереди
        ...
        // Очистка памяти под структуру очереди
        free(q);
    }
}
...
int queue_get(queue_t *q, int *val) {
	...
	if (q->count == 0)
		return 0;
//есть вероятность, что при конкурентной работе tmp = NULL
	qnode_t *tmp = q->first;
